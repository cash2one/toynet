...
//	memset(&g_AdminInfo,0,sizeof(g_AdminInfo));
	...
	if( !fileO.Open( "AdminInfo.cfg", CFile::modeRead, &e ) ){
		...
		fileO.Read(tmp,sizeof(g_AdminInfo));
		fileO.Close();
	}
	...
	for( i=0; i<MAX_ADMIN_NUM; i++){
		InsItem(g_AdminInfo.admin[i].ID, g_AdminInfo.admin[i].Grade, g_AdminInfo.admin[i].UserIP);
	}
...
void CAdminManager::OnOK() 
{
	...
	g_AdminInfo.nTotal = m_ctrlAdminList.GetItemCount();
	
	for(int i=0; i<g_AdminInfo.nTotal; i++){
		CString strID = m_ctrlAdminList.GetItemText(i,0);
		strncpy(g_AdminInfo.admin[i].ID,strID.operator LPCTSTR(),15);

		strID = m_ctrlAdminList.GetItemText(i,1);
		g_AdminInfo.admin[i].Grade = atoi(strID);

		strID = m_ctrlAdminList.GetItemText(i,2);
		strncpy(g_AdminInfo.admin[i].UserIP,strID.operator LPCTSTR(), 24);
	}
	...
	memcpy(tmp, &g_AdminInfo, sizeof(g_AdminInfo));

	for( i=0; i<sizeof(g_AdminInfo);i++)
	{
		tmp1[i]^= (tmp[i]^'C'^'Y');
	}
...
//	memcpy(&g_AdminInfo, tmp1, sizeof(g_AdminInfo));
	...
	fileS.Write(tmp1,sizeof(g_AdminInfo));
