...
/*
#ifdef _DEBUG
	#pragma comment(lib, "JpegLib_Debug.lib")
#else
	#pragma comment(lib, "JpegLib_Release.lib")
#endif
*/
...
BYTE * JpegFile::JpegFileToRGB(LPCTSTR fileName,
							   UINT *width,
							   UINT *height)

{
	...
	*/
	...
	if ((infile = fopen(fileName, "rb")) == NULL) {
		sprintf(buf, "파일열기 실패: %s\n", fileName);
		//AfxMessageBox(buf);
		return NULL;
	}

	BYTE* pRtn = JpegFileToRGB(infile, width, height);
...
BYTE * JpegFile::JpegFileToRGB(FILE *infile,
							   UINT *width,
							   UINT *height)
...
BOOL JpegFile::GetJPGDimensions(LPCTSTR fileName,
								UINT *width,
								UINT *height)

{
	...
	*/
	...
	BOOL bRtn = GetJPGDimensions(infile, width, height);
...
BOOL JpegFile::GetJPGDimensions(FILE* infile,
								UINT *width,
								UINT *height)
...
BOOL JpegFile::RGBToJpegFile(LPCTSTR fileName, 
							BYTE *dataBuf,
							UINT widthPix,
							UINT height,
							BOOL color, 
