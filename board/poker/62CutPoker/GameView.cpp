	...
	case WM_MOUSEMOVE :
		{
			...
			// 체팅 박스
			if(ChatBox.OnMouseMove(mxp, myp)==TRUE) return 1;
			if(Game.OnMouseMove(mxp, myp)==TRUE) return 1;

			if(CharBox[0].OnMouseMove(mxp, myp)==TRUE) return 1;
			if(CharBox[1].OnMouseMove(mxp, myp)==TRUE) return 1;
			if(CharBox[2].OnMouseMove(mxp, myp)==TRUE) return 1;
			if(CharBox[3].OnMouseMove(mxp, myp)==TRUE) return 1;
			if(CharBox[4].OnMouseMove(mxp, myp)==TRUE) return 1;
			if(g_RI.bPlayer != 0) {
				if(CharBox[5].OnMouseMove(mxp, myp)==TRUE) return 1;
			}
	
			//if(ExitBtn.OnMouseMove(mxp, myp)==TRUE) return 1;
			
			InviteBtn.OnMouseMove(mxp, myp); //초대
			ExitBtn.OnMouseMove(mxp, myp);   //종료
			ChangeDisplayBtn.OnMouseMove(mxp, myp); //모드
			OptionBtn.OnMouseMove(mxp, myp);//옵션

//			MasterBtn.OnMouseMove(mxp, myp);//옵션
			MinimizeBtn.OnMouseMove(mxp, myp);//옵션
			GameItemBtn.OnMouseMove(mxp, myp);//옵션

			CaptureBtn.OnMouseMove(mxp, myp);//옵션

			// ### [ 관전기능 ] ###
			sInOut_Btn.OnMouseMove(mxp, myp);
			ObserverViewBtn.OnMouseMove(mxp, myp);

			if(g_bChangeCardStage){
				ChangeCard_OK.OnMouseMove(mxp, myp);
				ChangeCard_Cancel.OnMouseMove(mxp, myp);
			}
			
			DieBtn.OnMouseMove(mxp, myp);
			CheckBtn.OnMouseMove(mxp, myp);
			CallBtn.OnMouseMove(mxp, myp);
			PingBtn.OnMouseMove(mxp, myp);
			DadangkBtn.OnMouseMove(mxp, myp);
			MaxBtn.OnMouseMove(mxp, myp);
...
/*
			...
			TipX = 0;
			TipY = 0;
			if(DieBtn.OnMouseMove(mxp, myp))
			{
				m_BtnMsgTip = "[Insert]";
				TipX =625;
				TipY =510; 
			}
			if(CheckBtn.OnMouseMove(mxp, myp))
			{
				m_BtnMsgTip = "[Home]";
				TipX =682;
				TipY =510;
			}
			if(CallBtn.OnMouseMove(mxp, myp))
			{
				m_BtnMsgTip = "[PageUp]";
				TipX =732;
				TipY =510;
			}
			if(PingBtn.OnMouseMove(mxp, myp))   
			{
				m_BtnMsgTip = "[Delete]";
				TipX =623;
				TipY =581;
			}
			if(DadangkBtn.OnMouseMove(mxp, myp)) 
			{
				m_BtnMsgTip = "[End]";
				TipX =687;
				TipY =581;
			}
			if( MaxBtn.OnMouseMove(mxp, myp) )   
			{
				m_BtnMsgTip = "[PageDn]";
				TipX =732;
				TipY =581;
...
/*
	...
	case WM_LBUTTONDOWN :
		{	
			...
			CRect rc;
			rc.SetRect( 0, 0, 530, 28 );
			POINT pt;
			pt.x = mxp;
			pt.y = myp;
			if( rc.PtInRect( pt ) )	{
				g_pMainDlg->SendMessage( WM_LBUTTONDOWN, MK_LBUTTON, lParam );
				return 1;
			}

			// 채팅창 클릭시 채팅모드
			rc.SetRect( 0, 0, ChatBox.m_Width-30, ChatBox.m_Height+20 ); //입력창 포함
			rc.OffsetRect(ChatBox.m_Xp, ChatBox.m_Yp);
			if(rc.PtInRect( pt) ) {
				ChatBox.SendEdit.SetFocus();
				return 1;
			}
		...
		//	if(GetFocus() != this) {
		//		SetFocus();
		//		if(Play[0].PlayState != 1) bLClick = FALSE; // 게임중
		//	}
			this->SetFocus();
			...
			// 캐릭터 보기는 항상 클릭 가능
			ChatBox.OnLButtonDown(mxp, myp);

			if(Game.OnLButtonDown(mxp, myp)==TRUE) return 1;
		
			
			if(g_RI.bPlayer != 0) {
				CharBox[5].OnLButtonDown(mxp, myp);
			}
			CharBox[4].OnLButtonDown(mxp, myp);
			CharBox[3].OnLButtonDown(mxp, myp);
			CharBox[2].OnLButtonDown(mxp, myp);
			CharBox[1].OnLButtonDown(mxp, myp);
			CharBox[0].OnLButtonDown(mxp, myp);
			
			
			InviteBtn.OnLButtonDown(mxp, myp);
			ExitBtn.OnLButtonDown(mxp, myp);
			ChangeDisplayBtn.OnLButtonDown(mxp, myp);
			OptionBtn.OnLButtonDown(mxp, myp);

//			MasterBtn.OnLButtonDown(mxp, myp);//옵션
			MinimizeBtn.OnLButtonDown(mxp, myp);//옵션
			GameItemBtn.OnLButtonDown(mxp, myp);//옵션
			CaptureBtn.OnLButtonDown(mxp, myp);//옵션

			if(g_bChangeCardStage){
				ChangeCard_OK.OnLButtonDown(mxp, myp);
				ChangeCard_Cancel.OnLButtonDown(mxp, myp);
			}

			MasterBtn.OnLButtonDown(mxp, myp);//옵션
			BanishvoteBtn.OnLButtonDown(mxp, myp);//옵션
//			SortBtn.OnLButtonDown(mxp, myp);

			// ### [ 관전기능 ] ###
			sInOut_Btn.OnLButtonDown(mxp, myp);
			ObserverViewBtn.OnLButtonDown(mxp, myp);
			

			if(bLClick) {
				// 게임 진행관련 버튼 진행중일때만 클릭
	...
	case WM_LBUTTONUP :
		{
			...
			ChatBox.OnLButtonUp(mxp, myp);
			
			if(g_RI.bPlayer != 0) {
				if(CharBox[5].OnLButtonUp(mxp, myp)==TRUE) return 1;
			}
			if(CharBox[4].OnLButtonUp(mxp, myp)==TRUE) return 1;
			if(CharBox[3].OnLButtonUp(mxp, myp)==TRUE) return 1;
			if(CharBox[2].OnLButtonUp(mxp, myp)==TRUE) return 1;
			if(CharBox[1].OnLButtonUp(mxp, myp)==TRUE) return 1;
			if(CharBox[0].OnLButtonUp(mxp, myp)==TRUE) return 1;
			
			InviteBtn.OnLButtonUp(mxp, myp);
			ExitBtn.OnLButtonUp(mxp, myp);
			ChangeDisplayBtn.OnLButtonUp(mxp, myp);
			OptionBtn.OnLButtonUp(mxp, myp);
			
			//				MasterBtn.OnLButtonUp(mxp, myp);//
			MinimizeBtn.OnLButtonUp(mxp, myp);//
			GameItemBtn.OnLButtonUp(mxp, myp);//
			CaptureBtn.OnLButtonUp(mxp, myp);//옵션

			if(g_bChangeCardStage){
				ChangeCard_OK.OnLButtonUp(mxp, myp);
				ChangeCard_Cancel.OnLButtonUp(mxp, myp);
			}
			
			// ### [ 관전기능 ] ###
			sInOut_Btn.OnLButtonUp(mxp, myp);
			ObserverViewBtn.OnLButtonUp(mxp, myp);

			MasterBtn.OnLButtonUp(mxp, myp);//
			BanishvoteBtn.OnLButtonUp(mxp, myp);//옵션
